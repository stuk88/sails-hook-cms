...
module.exports = function (sails) {
    ...
    var getModelName = function (item, config) {
        if (typeof item.name == "function") // support for model instance functions
            return item.name();
            return (config && config.title) ? item[config.title] : _.get(item,"name",item.id);
    }
    ...
    var populateAttrWithEnumOptions = function (attr) {
        let enum_options = attr.enum || _.get(attr,"validations.isIn", false);
        attr.options = enumAsOptions(enum_options);
        return attr;
    }
    ...
    return {
        list: {
            item: function (value, attrName, attrs) {
                ...
                if (attrs.type == 'datetime' || (attrs.type == "ref" && attrs.columnType == "datetime")) {
                    ...
                    return value && JSON.stringify(value).substring(0,100);
                } else if (attrs.type == "string" && value != null) {
        ...
        form: {
            getElement: async function (name, attr, value, model) {
                var ejsFormPartials = async (options) => {
                    options = {...options, _};
                    return await ejs.renderFile(path.join(__dirname, '../partials/forms.ejs'), options)
                };
                ...
                //If is STRING
                if (attr.type == 'string') {
                    if (attr.enum || _.get(attr,"validations.isIn", false)) {
                        return ejsFormPartials({
                            element: 'select',
                            name: name,
                            attr: attr,
                            value: value,
                            options: enumAsOptions(attr.enum || _.get(attr,"validations.isIn", false))
                        });
                    } else {
                        var attr_type = attr.type == "string" ? 'text' : attr.type;
                            attr_type = _.get(attr, "cms.type", attr_type);
                        return ejsFormPartials({
                            element: attr_type == "text" ? 'input' : attr_type,
                            type: attr_type,
                            name: name,
                            attr: attr,
                            value: value
                ...
                } else if (attr.type == 'date' || attr.type == 'datetime' || (attr.type == "ref" && attr.autoMigrations.columnType == "datetime")) {
                    return ejsFormPartials({
                        element: 'datetime',
                        name: name,
                        attr: attr,
                ...
                } else if ((attr.type == 'integer' || attr.type == 'float' || attr.type == 'number') && !attr.model) {
                    return ejsFormPartials({
                        element: 'input',
                        type: 'number',
                        name: name,
                    ...
                    // If is json
                } else if ((attr.type == 'json' || attr.type == 'text') && !(attr.cms && attr.cms.collection)) {
                    return ejsFormPartials({
                        element: 'json',
                        name: name,
                        attr: attr,
                        value: (attr.type == 'json') ? JSON.stringify(value) : value
                    });
                        ...
                        if (sails.models[attr.collection]) {
                            sails.models[attr.collection].find().exec(function (err, models) {
                                ...
                                resolve(ejsFormPartials({
                                    ...
                                    attr: attr,
                                    name: name,
                                    options: modelsAsOptions(models, sails.models[attr.collection].cms),
